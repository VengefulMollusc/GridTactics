...
public abstract class Movement : MonoBehaviour {
    ...
    // basic implementation to expand search based on range from character
    protected virtual bool ExpandSearch (Tile from, Tile to)
    {
        return (from.distance + 1) <= range;
    }

    // filter method loops through tiles returned by search and removes any which hold blocking content
    // may need to shift this step into ExpandSearch so that tiles past blocked tiles are not considered in range?
